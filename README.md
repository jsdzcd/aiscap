...
### 3. 使用步骤
1. 在侧边栏输入您的 **API Key**（如果使用本地模型如 Ollama，请修改 Base URL）。
2. 上传一个 `.pcap` 或 `.pcapng` 文件。
3. 点击 **“🚀 开始 AI 智能分析”** 按钮。
...
## 🔒 隐私与安全
本工具在本地解析 PCAP 文件，仅将提取的**摘要信息**（不含完整数据包）发送至您配置的 AI 接口。若需完全离线，建议配合 [Ollama](https://ollama.com/) 使用本地大模型。
